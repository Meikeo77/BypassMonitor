...
+ (instancetype)cachedPropertyWithProperty:(objc_property_t)property
{
    ...
    BMProperty *propertyObj = objc_getAssociatedObject(self, property);
    if (propertyObj == nil) {
...
- (void)setProperty:(objc_property_t)property
{
    ...
    BMExtensionAssertParamNotNil(property);
    ...
    _type = [BMPropertyType cachedTypeWithCode:code];
}
...
- (NSArray *)propertyKeysWithStringKey:(NSString *)stringKey
{
    ...
    for (NSString *oldKey in oldKeys) {
        ...
        if (start != NSNotFound) { // 有索引的key
            ...
            if (prefixKey.length) {
                BMPropertyKey *propertyKey = [[BMPropertyKey alloc] init];
            ...
            for (NSInteger i = 0; i<cmps.count - 1; i++) {
                BMPropertyKey *subPropertyKey = [[BMPropertyKey alloc] init];
                subPropertyKey.type = BMPropertyKeyTypeArray;
